...
abstract class AbstractCompany extends ActiveRecord implements CompanyInterface, RelationInterface
{
    ...
    public static function create(UserInterface $owner, string $role): CompanyInterface
    {
        ...
        if (!$company->insert()) {
            throw new Exception(Yii::t('app-sm-user', 'Ошибка при добавлении компании'));
        }
        $company->refresh();
        ...
        if (!(new CompanyUserHandler($company_user))->create()) {
            throw new Exception(Yii::t('app-sm-user', 'Ошибка при добавлении сотрудника в компанию'));
        }
        return $company;
    }
    ...
    public function rules(): array
    {
        return [
            ...
            [['name'], 'default', 'value' => Yii::t('app-sm-user', 'Моя компания')],
            [['name'], 'string', 'max' => 500],
            [['description'], 'string']
        ];
    }
    ...
    public function attributeLabels(): array
    {
        return [
            ...
            'owner_id' => Yii::t('app-sm-user', 'Владелец'),
            'name' => Yii::t('app-sm-user', 'Название'),
            'description' => Yii::t('app-sm-user', 'Описание'),
        ];
    }
